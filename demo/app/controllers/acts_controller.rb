class ActsController < ApplicationController
  ...
  # GET /acts.xml
  def index
    @acts = Act.find(:all, :order => "plandate desc, dcfcode, drfcode, itemcode, acttypecode")
  ...
  def create
    ...
    respond_to do |format|
      if @act.save
        flash[:notice] = I18n.t('layout.common.successfullycreated')
  ...
  def update
    ...
    respond_to do |format|
      if @act.update_attributes(params[:act])
        flash[:notice] = 'Act was successfully updated.'
        format.html { redirect_to(@act) }
        format.xml  { head :ok }
      else
